...
## Passive TV diagnostic add-on

`hdmirxtest-v1.5/TvDiag.sh` adds a passive one-shot TV diagnostic without changing the V1.5 TX test. Use `bash TvDiag.sh install` once, then `bash TvDiag.sh field` before moving the Orange Pi 5 Plus to a TV. The next boot records HDMI connector/EDID/DRM/PHY state for about 90 seconds and powers off automatically. Back in the office, `bash TvDiag.sh results` creates `~/hdmirxtest-tv-diag-latest.tar.gz`.
