...
RESULT_DIR=${RESULT_DIR:-/tmp/hdmirxtest-v120-${MODE}-${STAMP}}
...
Fast paths (no network fetch, clean rebuild, or pre-run debug dump):
  ...
  preparemonitor   Clone the connected HDMI-TX monitor EDID with a 1080p240 cap
...
Maintenance / diagnostics:
  ...
  prepare240        Load the bundled Zowie-derived 1080p240 EDID
  restoreedid       Restore the original HDMI-RX EDID
...
case "$MODE" in
  live|240|preparemonitor|sync|diagnostic|baseline|probe240|prepare240|restoreedid|debug) ;;
  *) usage; exit 2 ;;
esac
...
case "$MODE" in
  ...
  prepare240)
    set +e
    as_root env VIDEO="$VIDEO" \
      bash "$ROOT/scripts/prepare-240.sh" 2>&1 | tee "$RESULT_DIR/prepare-240.log"
    RUN_RC=${PIPESTATUS[0]}
    set -e
    ;;
