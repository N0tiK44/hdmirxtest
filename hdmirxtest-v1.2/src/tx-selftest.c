...
int main(int argc, char **argv)
{
    ...
    int option;
    while ((option = getopt_long(argc, argv, "c:s:e:h", options, NULL)) != -1) {
        switch (option) {
    ...
    drmModeModeInfo *mode = choose_mode(connector);
    uint32_t crtc_id = 0;
    ...
    if (!mode || !pipeline_found) {
        fprintf(stderr, "HDMI connector found, but no compatible CRTC/primary plane/mode exists.\n");
        drmModeFreeConnector(connector);
        drmModeFreeResources(resources);
        close(fd);
        return 1;
    ...
    fprintf(stdout, "mode=%s %ux%u refresh=%.3fHz pixel_clock=%ukHz\n",
        mode->name, mode->hdisplay, mode->vdisplay, refresh_hz(mode), mode->clock);
