...
# A persistent V1.2.3 service may already own V4L2/DRM. Stop it before the
# deliberate, source-disconnected EDID transaction.
if command -v systemctl >/dev/null 2>&1 &&
   systemctl is-active --quiet hdmirxtest-passthrough.service 2>/dev/null; then
  systemctl stop hdmirxtest-passthrough.service
fi
...
if command -v systemctl >/dev/null 2>&1 && [[ -d /run/systemd/system ]]; then
  ...
  install -m 0755 "$ROOT/scripts/service-run.sh" \
    /usr/local/libexec/hdmirxtest-service-run
  install -m 0644 "$ROOT/packaging/systemd/hdmirxtest-edid.service" \
    /etc/systemd/system/hdmirxtest-edid.service
  install -m 0644 "$ROOT/packaging/systemd/hdmirxtest-passthrough.service" \
    /etc/systemd/system/hdmirxtest-passthrough.service
  {
    ...
    printf 'BUFFERS=%s\n' "${BUFFERS:-4}"
    printf 'DURATION=0\n'
    printf 'AUTO_RESTART=1\n'
    printf 'WAIT_FOR_SOURCE=1\n'
    printf 'TARGET_REFRESH_MILLIHZ=auto\n'
    printf 'CSV=/var/tmp/hdmirxtest-live.csv\n'
  } >/etc/default/hdmirxtest
  systemctl daemon-reload
  systemctl enable hdmirxtest-edid.service hdmirxtest-passthrough.service >/dev/null
  systemctl restart hdmirxtest-edid.service
  BOOT_SERVICE="systemd EDID loader and persistent passthrough installed/enabled"
...
if command -v systemctl >/dev/null 2>&1 && [[ -d /run/systemd/system ]]; then
  echo "Then start permanent video with: bash scripts/pi-cycle.sh service-start"
fi
