...
if v4l2-ctl -d "$VIDEO" --query-dv-timings >/dev/null 2>&1; then
  ...
  echo "Unplug HDMI-RX once, run: bash scripts/pi-cycle.sh setup, then reconnect." >&2
  exit 10
fi

if (( NO_AUTO_APPLY == 1 )); then
  echo "Persistent service paused: run setup with HDMI-RX unplugged, then restart the service." >&2
  exit 10
fi
...
echo "Reconnect HDMI-RX, then rerun: bash scripts/pi-cycle.sh 240" >&2
exit 11
