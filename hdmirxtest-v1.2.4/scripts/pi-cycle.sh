...
RESULT_DIR=${RESULT_DIR:-/tmp/hdmirxtest-v124-${MODE}-${STAMP}}
...
Setup and recovery:
  bash setup.sh            First installation and monitor EDID setup
  bash PiCycle.sh setup    Re-scan a replacement HDMI-TX monitor
  bash PiCycle.sh debug    Collect diagnostics without starting video
  bash PiCycle.sh restoreedid

Before setup or restoreedid, physically unplug Windows from HDMI-RX.
Leave the target monitor connected to HDMI-TX.
EOF
}

case "$MODE" in
  240|live|setup|preparemonitor|prepare240|debug|diagnostic|baseline|probe240|restoreedid|help|-h|--help) ;;
  *) usage; exit 2 ;;
esac
...
case "$MODE" in
  ...
  diagnostic|baseline)
    ...
    if as_root env VIDEO="$VIDEO" bash "$ROOT/scripts/verify-bridge-edid.sh"; then
      ...
      (( TEST_DURATION == 0 )) && TEST_DURATION=120
      as_root env OUTDIR="$RESULT_DIR" DURATION="$TEST_DURATION" \
        BUFFERS="$BUFFERS" VIDEO="$VIDEO" CARD="$CARD" CONNECTOR="$CONNECTOR" \
        PLANE="$PLANE" TARGET_REFRESH_MILLIHZ="$TARGET" \
  ...
  live|240)
    # Remove any older background instance so this terminal exclusively owns
    # V4L2 and DRM. Normal V1.2.4 operation never starts a video service.
    if command -v systemctl >/dev/null 2>&1 &&
       as_root systemctl is-active --quiet hdmirxtest-passthrough.service 2>/dev/null; then
      echo "Stopping the old background video service..."
    ...
    if (( MONITOR_RC != 0 )); then
      ...
      if as_root env VIDEO="$VIDEO" bash "$ROOT/scripts/verify-bridge-edid.sh"; then
        ...
        set +e
        as_root env \
          DURATION="$DURATION" BUFFERS="$BUFFERS" VIDEO="$VIDEO" CARD="$CARD" \
          CONNECTOR="$CONNECTOR" PLANE="$PLANE" TARGET_REFRESH_MILLIHZ=auto \
          CSV="$RESULT_DIR/trace.csv" \
          bash "$ROOT/scripts/run.sh" 2>&1 | tee "$RESULT_DIR/run.log"
        RUN_RC=${PIPESTATUS[0]}
        set -e

        # SIGINT is the expected user interface, not a failed run.
