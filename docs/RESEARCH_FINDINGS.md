...
## V1.1 experiment design — not yet hardware verified

V1.1 intentionally does not change the V1.0 buffer-ownership/fence algorithm. Its first objective is to prove or disprove 1920×1080 at ~240 Hz on the exact RK3588 HDMI-RX stack.
...
The hypothesis is that if the existing one-refresh KMS completion boundary remains, a stable ~240-Hz end-to-end path should move that dominant interval from roughly 16.68 ms to roughly 4.17 ms. This must be established from hardware traces, not assumed from the requested refresh rate.
