...
usage() {
  cat <<'USAGE'
HDMIRXTEST V1.5 — OFFLINE PLUG-AND-PLAY HDMI-TX FIELD TEST

  bash TxTest.sh install   Install the offline TX probe service
  bash TxTest.sh arm       Arm ONE automatic field test for the next boot
  bash TxTest.sh field     Arm the next boot and power this Pi off now
  bash TxTest.sh disarm    Cancel an armed field test
...
V1.5 does not touch HDMI-RX. It adds a one-shot offline field-test mode so
the Pi can be moved to a TV with no Ethernet, terminal, or keyboard required.
USAGE
}
...
V1.5 TX FIELD TEST INSTALLED.
The service is enabled but stays idle unless a one-shot field test is armed.
HDMI-RX remains deliberately unused.

From the office, use:
    bash TxTest.sh field

The Pi will power off. Move it to the TV, connect ONE HDMI-TX cable and power.
No Ethernet, terminal or keyboard is required. The next boot tests the TV once,
saves all evidence, then powers the Pi off automatically.
MSG
    ;;
  ...
  arm)
    ...
    enable_service
    echo "One offline HDMI-TX field test is armed for the NEXT boot."
    echo "It will consume the arm marker, save the test history, and power off automatically."
    ;;
  ...
  field)
    ...
    printf 'armed_at_utc=%s\nauto_poweroff=1\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" | \
      as_root tee "$ARM_FILE" >/dev/null
    enable_service
    echo "Field test armed. Syncing disks and powering off this Pi now..."
    sync
    sleep 1
    if command -v systemctl >/dev/null 2>&1 && [[ -d /run/systemd/system ]]; then
      as_root systemctl poweroff
    else
      as_root poweroff
    fi
    ;;
  ...
  disarm)
    ...
    echo "Offline HDMI-TX field test disarmed."
    ;;
  ...
  status)
    ...
    if as_root test -e "$ARM_FILE"; then
      echo "FIELD TEST: ARMED for next boot"
      as_root cat "$ARM_FILE" 2>/dev/null || true
    else
      echo "FIELD TEST: not armed"
    fi
    echo
